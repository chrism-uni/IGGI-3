...
public abstract class AbstractPlayer {
     ...
     */
    private Action lasAction = null;

    /**
     * Picks an action. This function is called every game step to request an
     * action from the player. The action returned must be contained in the
     * actions accessible from ActionMap, or no action
     * will be applied.
     * @param gameState Observation of the current state.
     * @param elapsedTimer Timer when the action returned is due.
     * @return An action for the current state
     */
    public abstract Action act(SimpleBattle gameState, ElapsedCpuTimer elapsedTimer);
