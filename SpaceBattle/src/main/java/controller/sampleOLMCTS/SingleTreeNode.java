...
public class SingleTreeNode
{
    ...
    public SingleTreeNode expand(SimpleBattle state) {
        ...
        //Roll the state
        if(playerID==0)
            state.advance(ActionMap.ActionMap[bestAction], ActionMap.ActionMap[this.m_rnd.nextInt(NUM_ACTIONS)]);
        else
            state.advance(ActionMap.ActionMap[this.m_rnd.nextInt(NUM_ACTIONS)], ActionMap.ActionMap[bestAction]);
    ...
    public SingleTreeNode uct(SimpleBattle state) {
        ...
        //Roll the state:
        if(playerID==0)
            state.advance(ActionMap.ActionMap[selected.childIdx], ActionMap.ActionMap[this.m_rnd.nextInt(NUM_ACTIONS)]);
        else
            state.advance(ActionMap.ActionMap[this.m_rnd.nextInt(NUM_ACTIONS)], ActionMap.ActionMap[selected.childIdx]);
    ...
    public double rollOut(SimpleBattle state)
    {
        ...
        while (!finishRollout(state,thisDepth)) {
            ...
            if(playerID==0)
                state.advance(ActionMap.ActionMap[action], ActionMap.ActionMap[this.m_rnd.nextInt(NUM_ACTIONS)]);
            else
                state.advance(ActionMap.ActionMap[this.m_rnd.nextInt(NUM_ACTIONS)], ActionMap.ActionMap[action]);
            thisDepth++;
        }
