...
import controller.Arnie.ArnieCoevSearch;
import controller.Arnie.ArnieSearch;
import core.Action;
 ...
 * Date: 17/10/12
 */
public class ArnieController implements BattleController {
    /**
     * Number of macro-actions that form the random path.
     */
   //public static int ARNIE_NUM_ACTIONS_INDIVIDUAL = 20; //8
    /**
     * Number of single actions that form a macro action.
     */
    //public static int ARNIE_MACRO_ACTION_LENGTH = 1; //15
    /**
     * Total number of evaluations can be used
     */
     ...
     */
    private ArnieCoevSearch m_search;
     ...
     */
    public ArnieController(ArnieCoevSearch searchAlg)
    {
        m_resetRS = true;
        m_search = searchAlg;
    ...
    public Action getAction(SimpleBattle game, int playerId, ElapsedCpuTimer elapsedTimer)
    {
        ...
        if(ArnieSearch.MACRO_ACTION_LENGTH == 1)
        ...
        if(cycle == 0)
        {
            ...
            m_currentMacroAction = ArnieSearch.MACRO_ACTION_LENGTH-1;
        }else
        {
    ...
    public void prepareGameCopy(SimpleBattle game)
    {
        ...
        if(m_lastMacroAction != -1)
        {
            ...
            int first = ArnieSearch.MACRO_ACTION_LENGTH - m_currentMacroAction - 1;
            for(int i = first; i < ArnieSearch.MACRO_ACTION_LENGTH; ++i)
            {
