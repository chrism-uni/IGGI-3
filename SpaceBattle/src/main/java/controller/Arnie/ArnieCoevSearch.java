 ...
 * Date: 17/10/12
 */
public class ArnieCoevSearch extends ArnieSearch {
    ...
    public static int NUM_INDIVIDUALS = 100;


    public ArnieGAIndividual[] m_individuals;

    public ArnieGAIndividual[] m_individualsOpp;
    ...
    ArnieICrossover cross;
    ArnieIMutation mut;
    ArnieISelection sel;
    ArnieICoevPairing pair;


    /**
     * Constructor of the random search engine.
     */
    public ArnieCoevSearch(ArnieICrossover ic, ArnieIMutation im, ArnieISelection is, ArnieICoevPairing icp, Random rnd)
    {
        super(rnd);
        cross = ic;
        mut = im;
    ...
    public void init(SimpleBattle gameState, int _playerID)
    {
        ...
        this.numEvals = 0;
        m_individuals = new ArnieGAIndividual[NUM_INDIVIDUALS];
        m_individualsOpp = new ArnieGAIndividual[NUM_INDIVIDUALS];
        this.playerID = _playerID;

        for(int i = 0; i < NUM_INDIVIDUALS; ++i) {
            m_individuals[i] = new ArnieGAIndividual(ArnieSearch.NUM_ACTIONS_INDIVIDUAL, playerID, this);
            m_individuals[i].randomize(m_rnd, ActionMap.ActionMap.length);

            m_individualsOpp[i] = new ArnieGAIndividual(ArnieSearch.NUM_ACTIONS_INDIVIDUAL, -1, null);
    ...
    public int run(SimpleBattle _m_currentGameState, ElapsedCpuTimer elapsedTimer)
    {
        ...
        while(remaining > 2*avgTimeTaken && remaining > remainingLimit)
        {
            ...
            ArnieGAIndividual[] nextOppPop = new ArnieGAIndividual[m_individualsOpp.length];
            int i;
            ...
            ArnieGAIndividual[] nextPop = new ArnieGAIndividual[m_individuals.length];
    ...
    private ArnieGAIndividual breed(ArnieGAIndividual[] pop)
    {
        ArnieGAIndividual gai1 = sel.getParent(pop, null);        //First parent.
        ArnieGAIndividual gai2 = sel.getParent(pop, gai1);        //Second parent.
     ...
     */
    private void printPopulation(ArnieGAIndividual[] pop)
    {
